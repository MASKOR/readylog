...
ifeq ($(HAVE_ECLIPSE),1)
   ...
   #$(warning blub)
   ECLIPSE_LOC = $(shell which eclipse)
   ...
   ifeq ($(ECLIPSE_CLP),1)
     ...
     ECLIPSE_ARCH    = $(shell eclipse -e "get_flag('hostarch',V),write(V)")
     ECLIPSE_PATH    = $(shell eclipse -e "get_flag('installation_directory',V),write(V)")
     ECLIPSE_VERSION = $(shell eclipse -e "get_flag('version',V),write(V)")
   else
