 ...
 * ***************************************************************************
 *
 *           $Id: steadylog.pl 214 2009-06-02 14:53:47Z pannhausen $
 *         @date: 06.03.03
 ...
 * last modified: $Date: 2009-06-02 16:53:47 +0200 (Di, 02. Jun 2009) $
 *            by: $Author: pannhausen $
 *
 * **************************************************************************/

:- write(" -----> Loading steadylog.pl \t ... \n").
 ...
 */
:- ensure_loaded('st-final_trans.pl').
...
holds(not(P),H)      :- !, (
			     ...
			     holds_not(P,H)
			   ; not holds(P,H) ).
holds(constraint(Ac,meets,Tr,A),H) :- !, 
    printColor( cyan, "<holds(constraint)>\n", [] ),
    printColor( cyan, "  Ac = '%w'\n", [Ac] ),
    printColor( cyan, "  Tr = '%w'\n", [Tr] ),
    printColor( cyan, "  A  = '%w'\n", [A] ),
    printColor( cyan, "  H  = '%w'\n", [H] ),
    ( 
        starter(Tr,Ts), 
        printColor( cyan, "  starter(%w,%w)\n", [Tr,Ts] ),
        finisher(Ac,Af), 
        printColor( cyan, "  finisher(%w,%w)\n", [Ac,Af] ),
        (
            ( 
                A = Ts 
            )
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
:- writeln(" <----- Loading steadylog.pl \t[DONE]\n").
