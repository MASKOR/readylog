 ...
 * ***************************************************************************
 *
 *           $Id: final_trans.pl 215 2009-06-02 15:46:11Z pannhausen $
 *       @author: Christian Fritz <Christian.Fritz@rwth-aachen.de>
 *   description: TRANS and FINAL of Readylog Interpreter 
 * last modified: $Date: 2009-06-02 17:46:11 +0200 (Di, 02. Jun 2009) $
 *            by: $Author: pannhausen $
 *
 * **************************************************************************/
...
transPr(pconc(E1,E2),S,pconc(EE1,EE2),SS,P) :- 
	...
	(
	  ...
	  ( /*  if no time consumed by action,
	        ...
	        write('trying pconc1.1'),nl,
	  simultaneous(S,S3) ->write('trying pconc1.1a'),nl,
	      EE1=E3, EE2=E2, SS=S3, P=P3
	  ;
	      (transPr(E2,S,E4,S4,P4) ->
		      (write('trying pconc1.2'),nl,earliereq(S3,S4) ->
		          EE1=E3, EE2=E2, SS=S3, P=P3
		      ;
...
%transPr( E, S, [], [E_sub|S], 1) :- 
	...
	; 
	  ...
	  ;                
	    % vvv AW vvv
	    % If there are any constraints bound to E, we'll check them and try to
	    % resolve them.
        ( 
            ( 
                needs_attention( E, S, C )
            ) 
            -> 
            (
                !,
                printColor( blue, "  todo = %w\n", [C] ),
                C = [Ac,Pi,Tr],
                apply_constraint(E, Ac,Pi,Tr,S,E1,S1),
                %subf(E,E_sub,S) , 
                %E1 = [start_calibrate, stop_calibrate, E] ,
                %S1 = S,
                printColor( blue, "  program remainder  = %w\n", [E1] ),
                printColor( blue, "  resulting situation = %w\n", [S1] )
            )                
        )  
        % If there are no constraints bound to E, just use the next branch of 
        % this disjunction vvv
	  ;
	    /* substitute current values for all fluents
	    before appending to situation */
	    %printColor( pink, "E     = '%w'\n", [E] ),
	    %printColor( pink, "E_sub = '%w'\n", [E_sub] ),
	    %printColor( pink, "S     = '%w'\n", [S] ),
	    subf(E,E_sub,S), 
	    printColor( blue, "  E_sub = '%w'\n", [E_sub] ),
	    % AW
	    E1 = [] ,
	    S1 = [E_sub|S],
	    printColor( blue, "  E1    = '%w'\n", [E1] ),
	    printColor( blue, "  S1    = '%w'\n", [S1] )	   
	  )
	),
	dbgFinishTrans,
	printColor( blue , "</transPr>\n", [] ).
